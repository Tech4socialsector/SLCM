...
def handle_upload_error(applicant_name, document_type, error_message):
    frappe.log_error(
        f"Upload failed for {applicant_name} | {document_type} | {error_message}",
...
def handle_session_timeout(applicant_name):
    ...
    return {
        ...
        "message": "Your session expired. Your draft has been saved. Please login again.",
...
def log_system_error(error_type, error_message, applicant_name=None):
    frappe.log_error(
        f"Type: {error_type} | Applicant: {applicant_name} | {error_message}",
