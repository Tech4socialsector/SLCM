...
def send_status_notification(applicant_name, new_status):
    ...
    message_map = {
        ...
        """,
        "Offered": f"""
            You have received an offer of admission for <b>{applicant.program}</b>.<br>
            Please login to your dashboard to accept the offer before the deadline.
        """,
        "Accepted": f"""
            Your admission to <b>{applicant.program}</b> has been confirmed.<br>
            Please complete the fee payment and document verification process.
        """,
        "Rejected": f"""
        ...
        """,
        "Waitlisted": f"""
            You have been placed on the waitlist for <b>{applicant.program}</b>.<br>
            You will be notified if a seat becomes available.
        """
    }
    subject = subject_map.get(new_status, "Application Status Update")
    message = message_map.get(new_status, "Your application status has been updated.")
    try:
        frappe.sendmail(
        ...
        frappe.log_error(
            f"Notification sent to {applicant.email} for status: {new_status}",
            "Notification Log"
        )
...
def send_offer_deadline_reminder(applicant_name, campus, days_left):
    ...
    frappe.sendmail(
        ...
        If you do not respond before the deadline, your offer will be
        automatically cancelled.<br><br>
...
def send_document_reminder(applicant_name, missing_docs):
    ...
    frappe.sendmail(
        ...
        message=f"""
        Dear {applicant.candidate_name},<br><br>
        The following documents are still pending for your application
        <b>{applicant.application_id}</b>:<br>
        <ul>{doc_list}</ul>
