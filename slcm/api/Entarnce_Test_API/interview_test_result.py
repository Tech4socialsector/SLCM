...
def check_interview_staff_permission():
	"""Ensures the caller is logged in and has an administrative / Interview Staff role."""
	user = frappe.session.user
	if user == "Guest":
		frappe.throw(_("Not permitted"), frappe.PermissionError)

	user_roles = frappe.get_roles(user)
	allowed_roles = ["Exam Cell", "System Manager", "Interview Staff Member", "Entrance Test Admin", "Administrator"]
...
def upload_interview_result():
	...
	Requires Exam Cell / Interview Staff / Admin role.
	"""
	check_interview_staff_permission()
...
def get_interview_result_by_applicant(applicant):
	...
	Requires authentication and ownership or Interview Staff / Admin role.
	"""
	user = frappe.session.user
	if user == "Guest":
		frappe.throw(_("Not permitted"), frappe.PermissionError)
	...
	try:
		...
		user_roles = frappe.get_roles(user)
		is_staff = any(role in user_roles for role in ["Exam Cell", "System Manager", "Interview Staff Member", "Entrance Test Admin", "Administrator"]) or user == "Administrator"

		if not is_staff:
			alloc_email = frappe.db.get_value("Interview Seat Allocation", applicant, "email")
