frappe.ui.form.on("Improvement Exam Registration", {
    refresh(frm) {
        ...
        frm.add_custom_button(__("View Payment Log"), function () {
            frappe.set_route("List", "Improvement Exam Payment Log", {
                improvement_exam_registration: frm.doc.name,
            });
        ...
        if (["Paid", "Captured"].includes(frm.doc.payment_status)) {
            frm.add_custom_button(__("Download Receipt"), function () {
                const url = `/printview?doctype=Improvement%20Exam%20Registration&name=${encodeURIComponent(frm.doc.name)}&format=Improvement%20Exam%20Receipt&trigger_print=0`;
                ...
                frappe.prompt(
                    ...
                    function (values) {
                        frappe.call({
                            ...
                            args: {
                                doctype: "Improvement Exam Registration",
                                name: frm.doc.name,
                                fieldname: {
                                    payment_status:    "Paid",
