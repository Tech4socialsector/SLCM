...
class IDCardGeneration(Document):
	...
	def validate(self):
		...
		if not self.is_new():
			self._old_card_status = frappe.db.get_value("ID Card Generation", self.name, "card_status")
			self._old_qr_data = frappe.db.get_value("ID Card Generation", self.name, "qr_code_data")
		else:
