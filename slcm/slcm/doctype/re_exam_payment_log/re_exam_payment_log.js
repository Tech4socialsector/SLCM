...
frappe.ui.form.on("Re Exam Payment Log", {
    refresh(frm) {
        ...
        // ── Download Receipt (only when payment is Paid/Captured) ─────
        if (["Paid", "Captured"].includes(frm.doc.payment_status) && frm.doc.re_exam_registration) {
            frm.add_custom_button(__("Download Receipt"), function () {
                const url = `/printview?doctype=Re%20Exam%20Registration&name=${encodeURIComponent(frm.doc.re_exam_registration)}&format=Re%20Exam%20Receipt&trigger_print=0`;
