    ...
    if has_old and has_new:
        ...
        frappe.db.sql("""
            ...
            WHERE (`batch_year_ref` IS NULL OR `batch_year_ref` = '')
              ...
              AND `data_xgxm` != ''
        """)
        # Commit DML before DDL – Frappe raises ImplicitCommitError if DDL
        # runs inside an open transaction (DDL causes implicit commit in MariaDB)
        frappe.db.commit()
        ...
        # Commit any open transaction before DDL
        frappe.db.commit()
        frappe.db.sql(
