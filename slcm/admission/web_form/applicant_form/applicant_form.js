...
// ───────────────────────────────────────────────────────────────────
//  FILE UPLOADER — SLCM defaults for Applicant Web Form
// ───────────────────────────────────────────────────────────────────

/** Hide the "This file is public" warning — CSS only, no JS DOM walking. */
(function () {
	if (document.getElementById('slcm-apf-pub-warn')) return;
	var s = document.createElement('style');
	s.id = 'slcm-apf-pub-warn';
	s.textContent =
...
/** Wrap frappe.ui.FileUploader to enforce SLCM upload defaults. */
function _slcmApfWrapUploader() {
	if (!window.frappe || !frappe.ui || !frappe.ui.FileUploader) return;
	// Skip if the global file_uploader_globals.js already patched it
	if (frappe.ui.FileUploader._slcmFileUploaderDefaults) return;
	if (frappe.ui.FileUploader._slcmApfWrapped) return;

	var _Orig = frappe.ui.FileUploader;

	function _SlcmApfUploader(opts) {
		opts = opts || {};
		opts.is_private = 0;
		...
		return new _Orig(opts);
	}
	_SlcmApfUploader.UploadOptions = _Orig.UploadOptions;
	_SlcmApfUploader._slcmApfWrapped = true;
	frappe.ui.FileUploader = _SlcmApfUploader;
}

(function () {
	var _n = 0;
	var _t = setInterval(function () {
		_slcmApfWrapUploader();
		var done = window.frappe && frappe.ui && frappe.ui.FileUploader &&
			(frappe.ui.FileUploader._slcmApfWrapped ||
				frappe.ui.FileUploader._slcmFileUploaderDefaults);
		if (done || ++_n > 100) clearInterval(_t);
	}, 120);
	if (window.frappe && typeof frappe.require === 'function') {
		frappe.require('file_uploader.bundle.js', _slcmApfWrapUploader);
	}
})();
