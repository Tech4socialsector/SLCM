...
def execute(filters: dict | None = None):
	...
	total_attended = sum(d.get("attended_count", 0) for d in data)

	summary = [
		{
			"label": _("Total Attended Applicants"),
	...
	message = (
		_("Attended applicant counts by centre.")
		if data
		else _("No attended applicants found.")
	)
...
def get_data(filters: dict) -> list[dict]:
	...
	if (
		...
		if not provider_name:
			# User has Provider role but no linked Entrance Test Provider document -> return empty
			return []
		provider_filter = provider_name
	elif filters.get("entrance_test_provider"):
		provider_filter = filters.get("entrance_test_provider")

	conditions = ["entrance_test_status = 'Attended'", "docstatus < 2"]
	values = {}
