...
def execute(filters: dict | None = None):
	...
	total_scheduled = sum(d.get("scheduled_count", 0) for d in data)

	summary = [
		{
			"label": _("Total Scheduled Applicants"),
	...
	message = (
		_("Scheduled applicant counts by centre.")
		if data
		else _("No scheduled entrance tests found.")
	)
...
def get_data(filters: dict) -> list[dict]:
	...
	# Check if user is an Entrance Test Provider (and not System Manager / Entrance Test Admin)
	...
	if (
		...
		if not provider_name:
			# User has Provider role but no linked Entrance Test Provider document
			return []
		provider_filter = provider_name
	elif filters.get("entrance_test_provider"):
		provider_filter = filters.get("entrance_test_provider")

	conditions = ["entrance_test_status = 'Scheduled'", "docstatus < 2"]
	values = {}
